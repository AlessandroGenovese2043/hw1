...
$query = "SELECT * FROM (prefs JOIN users ON prefs.user = users.id) JOIN posts ON prefs.post = posts.idpost where prefs.user = '$username'";
...
if(mysqli_num_rows($res) > 0){
    while($result = mysqli_fetch_assoc($res)){
        ...
        $array_post[] = array('idprefs'=> $result['idpref'],'iduser' => $result['user'], 'idpost' => $result['post'],
                            ...
                            'number_posts' => $result['number_posts'], 'utente_loggato' => $username,'descrizione' => $result['descrizione'],
                            'tips' => $result['tips'],'image' => $result['image'], 'correct' => true);
